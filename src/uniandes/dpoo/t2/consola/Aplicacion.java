...
import uniandes.dpoo.t2.modelo.Bebida;
...
public class Aplicacion {
    ...
    private void mostrarMenuBebidas() {
    	System.out.println("\nMenú de Bebidas:\n");
        List<Bebida> bebida = restaurante.getBebidas();
        int indice = 1;
        for (Bebida producto : bebida) {
            System.out.println(indice + ". " + producto.toString());  // Usamos el método toString de ProductoMenu
            indice++;
        }
    }
    ...
    private void consultarPedido() {
        ...
        if (pedido != null) {
            ...
            for (ProductoAjustado productoAjustado : pedido.getProductosAjustados()) {
                System.out.println("Producto: " + productoAjustado.getProductoBase().getNombre());
                for (Ingrediente ing : productoAjustado.getIngredientesAdicionales()) {
                    System.out.println("\tIngrediente añadido: " + ing.getNombre() + " - Calorías: " + ing.getCalorias());
                }
            }
            
            System.out.println("\nCombos en el Pedido:");
            for (Combo combo : pedido.getCombos()) {
                System.out.println("Combo: " + combo.getNombre() + " - Calorías: " + combo.getCalorias());
            }

            System.out.println("\nBebidas en el Pedido:");
            for (Bebida bebida : pedido.getBebidas()) {
                System.out.println("Bebida: " + bebida.getNombre() + " - Calorías: " + bebida.getCalorias());
            }
            
            System.out.println("\nPrecio neto del pedido: " + pedido.getPrecioNetoPedido());
            System.out.println("Precio total del pedido (con IVA): " + pedido.getPrecioTotalPedido());
            System.out.println("\nTotal Calorías del Pedido: " + pedido.calcularTotalCalorias());
    ...
    public void agregarElementoAPedido() {
        ...
        int idPedido = Integer.parseInt(input("Ingrese el ID del pedido al que quiere agregar un producto, combo o bebida"));
        Pedido pedido = restaurante.getPedidoPorId(idPedido);

        if (pedido != null && !pedido.estaCerrado()) {
            System.out.println("1. Agregar Producto");
            System.out.println("2. Agregar Combo");
            System.out.println("3. Agregar Bebida"); // Opción para agregar bebidas
            int eleccion = Integer.parseInt(input("Elija una opción (1, 2 o 3):"));
            ...
            if (eleccion == 1) {
                ...
                pedido.agregarProductoAjustado(productoAjustado); 
                ...
                mostrarMenuBebidas();  // Método que muestra las bebidas disponibles
                int opcionBebida = Integer.parseInt(input("Seleccione una bebida del menú para agregar al pedido"));
                Bebida bebida = restaurante.getBebidas().get(opcionBebida - 1);  // Suponemos que el restaurante tiene un método getBebidas() que retorna una lista de bebidas
                pedido.agregarBebida(bebida);
                System.out.println("Bebida añadida con éxito al pedido.");
            } else {
